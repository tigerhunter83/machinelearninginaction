...
def testmnist():
    ...
    classifierArray, aggClassEst = adaBoostTrainDS(X_train, y_train, 10)
    end_time = dt.datetime.now() 
    print('Stop learning {}'.format(str(end_time)))

    prediction10 = adaClassify(X_test, classifierArray)
    errArr = mat(ones((len(X_test), 1)))
    errArr[prediction10 != mat(y_test).T].sum()
    plotRoc(aggClassEst.T, y_train)

def main():
    # print("hello")
    # dataArr, labelArr =  loadDataSet('horseColicTraning2.txt')
