...
def testmnist(kTup=('rbf', 10)):
    ...
    b, alphas = smoP(X_train, y_train, 200, 0.0001, 10000, kTup)
    ...
    for i in range(m):
        kernelEval = kernelTrans(sVs, datMat[i, :], kTup)
        predict = kernelEval.T * multiply(labelSV, alphas[svInd]) + b
        if sign(predict) != sign(y_train[i]): errorCount += 1
    print("the training error rate is: %f" % (float(errorCount) / m))
    ...
    for i in range(m):
        kernelEval = kernelTrans(sVs, datMat[i, :], kTup)
        predict = kernelEval.T * multiply(labelSV, alphas[svInd]) + b
        if sign(predict) != sign(y_test[i]): errorCount += 1
    print("the test error rate is: %f" % (float(errorCount) / m))
...
def main():
    ...
    # ws = calcWs(alphas, dataArr, labelsArr)

    # dataMat = mat(dataArr)
    # if (dataMat[0] * mat(ws) + b) > 0:
    #     print("属于1类")
    # else:
    #     print("属于-1类")
