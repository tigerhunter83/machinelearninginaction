...
def textParse(fileName):
    ...
    with tokenize.open(fileName) as f:
        ...
        for sentence in sentences:
            listOfTokens.extend([tok.lower() for tok in nltk.word_tokenize(sentence) if len(tok) > 2])
    return listOfTokens
...
def testmnist():
    ...
    p0V, p1V, p_1 = trainNB0(array(X_train), array(y_train))

    errorCount = 0
    for i in range(len(X_test)):
        vector = X_test[i]
        if classifyNB(array(vector), p0V, p1V, p_1) != y_test[i]:
            errorCount += 1
        
    print('the error rate is ', float(errorCount)/len(X_test))
